...
from PrinceXmusic.modules.play import arq
from PrinceXmusic.modules.play import generate_cover
from PrinceXmusic.modules.play import cb_admin_check
from PrinceXmusic.modules.play import transcode
from PrinceXmusic.modules.play import convert_seconds
from PrinceXmusic.modules.play import time_to_seconds
from PrinceXmusic.modules.play import changeImageSize
from PrinceXmusic.config import BOT_NAME as bn
from PrinceXmusic.config import DURATION_LIMIT
from PrinceXmusic.config import UPDATES_CHANNEL as updateschannel
from PrinceXmusic.config import que
from PrinceXmusic.function.admins import admins as a
from PrinceXmusic.helpers.errors import DurationLimitError
from PrinceXmusic.helpers.decorators import errors
from PrinceXmusic.helpers.admins import get_administrators
...
from PrinceXmusic.helpers.gets import get_file_name
from PrinceXmusic.services.callsmusic import callsmusic
from PrinceXmusic.services.callsmusic import client as USER
from PrinceXmusic.services.converter.converter import convert
...
async def playlist(client, message):
    try:
      ...
      message.reply("Chat not linked I guess")
      return
    global que
    queue = que.get(lol)
    if not queue:
...
async def ee(client, message):
    try:
      ...
      await message.reply("Chat not linked I guess")
      return
    queue = que.get(lol)
...
async def jiosaavn(client: Client, message_: Message):
    ...
    try:
      ...
      await message_.reply("Chat not linked I guess")
      return
    try:
      administrators = await get_administrators(conv)
    except:
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        f"<b>🔴 Flood Wait Error 🔴 \nUser {user.first_name} couldn't join your channel due to heavy requests for userbot! Make sure user is not banned in group."
                        "\n\nOr manually add @PrinceX_MusicAssistant to your Group and try again</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
